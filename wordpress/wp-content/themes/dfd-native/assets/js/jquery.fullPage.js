    ...
    $.fn.fullpage = function(options) {
        ...
        */
        ...
        function reBuild(resizing){
			...
			if(container.parent().hasClass('dfd-enable-onepage-animation') || container.parent().hasClass('dfd-disable-onepage-animation')) {
        ...
        */
        function performMovement(v){
			var animatioEffect = container.parent().hasClass('dfd-enable-onepage-animation');
			...
			if(animatioEffect) {
				var anim = container.parent().attr('class');
...
(function($) {
	"use strict";
	var $container = $('#layout.one-page-scroll .wpb-content-wrapper'),
	...
	if($container.length > 0) {
		if($headerContainer.length > 0 && $container.parent().data('smart-header') && $container.parent().data('smart-header') == '1') {
			$headerContainer.addClass('dfd-smart-header');
			smartHeader = true;
			if($container.parent().data('light-logo')) {
				lightLogo = $container.parent().data('light-logo');
			}
			if($container.parent().data('dark-logo')) {
				darkLogo = $container.parent().data('dark-logo');
			}
		}
		...
		$(document).ready(function() {
			dots = $container.parent().data('enable-dots');
			dfdInitOnePage();
		});
		
		$(window).load(function() {
			if($('#footer-wrap').length > 0) {	
				var footerIScroll = new IScroll($('#footer-wrap').get(0), {
					bindToWrapper: true,
					disablePointer: true,
					disableTouch: false,
					disableMouse: false,
				...
				footerIScroll.on('scrollEnd', function () {
					if (this.directionY == -1 && this.y == 0 && this.startY == 0 && $('body').hasClass('dfd-one-page-footer-expanded')) {
						$('body').removeClass('dfd-one-page-footer-expanded');
					}
				});
				$('body').on('post-load', function () {
					footerIScroll.refresh();
				});
			}
