...
//		if(session_id() == '' || (function_exists('session_status') && session_status() == PHP_SESSION_NONE)) {
		if(!is_admin() && (session_id() == '' || (function_exists('session_status') && session_status() == PHP_SESSION_NONE))) {
			@session_start();
		}
	}
}
...
if ( version_compare( $wp_version, '5.2', '>=' ) ) {
	add_filter('login_headertext', 'dfd_change_title_on_logo');
} else {
...
if(!function_exists('dfd_add_google_plus_meta')) {
	...
	function dfd_add_google_plus_meta() {
		...
		if( is_single() ) {
			...
			<?php
			if(!empty($thumbnail) && isset($thumbnail[0])) {
				echo '<meta itemprop="image" content="' . esc_url( $thumbnail[0] ) . '">' . "\n";
			}
			?>
...
if(is_admin()) {
	add_action('wp_default_scripts','colorpickerAlphaBugErrorFix');
	function colorpickerAlphaBugErrorFix($scripts) {
		$scripts->add( 'wp-color-picker', "/wp-admin/js/color-picker.js", array( 'iris' ), false, 1 );
		did_action( 'init' ) && $scripts->localize(
			'wp-color-picker',
			'wpColorPickerL10n',
			array(
				'clear'            => esc_attr__('Clear', 'dfd-native'),
				'clearAriaLabel'   => esc_attr__('Clear color', 'dfd-native'),
				'defaultString'    => esc_attr__('Default', 'dfd-native'),
				'defaultAriaLabel' => esc_attr__('Select default color', 'dfd-native'),
				'pick'             => esc_attr__('Select Color', 'dfd-native'),
				'defaultLabel'     => esc_attr__('Color value', 'dfd-native'),
			)
		);
	}
}