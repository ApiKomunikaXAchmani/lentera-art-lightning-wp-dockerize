...
if(!class_exists('DfdMetaBoxSettings')) {
	...
	class DfdMetaBoxSettings {
		...
		public static function get($name) {
			...
			if (isset($post) && !empty($post->ID) && !is_archive() && !is_search() && !is_404() && !is_home()) {
	...
	class Dfd_Theme_Helpers {
		...
		public static function generate_images_html($id = '', $w = '', $h = '') {
			...
			if($id != '') {
				...
				if(!$image_src) {
					$image_src = array(
						get_template_directory_uri() . '/assets/images/no_image_resized_120-120.jpg',
						'120',
						'120'
					);
				}
...
//			}
		...
		public static function dfd_get_image_alt( $img_alt ) {
			if ( empty( $img_alt['id'] ) ) {
				// For `Insert From URL` images.
				return isset( $img_alt['alt'] ) ? trim( strip_tags( $img_alt['alt'] ) ) : '';
			}

			$attachment_id = $img_alt['id'];
			if ( ! $attachment_id ) {
				return '';
			}
		...
		public static function dfd_get_bgposition_column() {
			return array(
				esc_html__('Default', 'dfd-native') => '',
				esc_html__('Left Top', 'dfd-native') => 'left top',
				esc_html__('Left Center','dfd-native') => 'left center',
				esc_html__('Left Bottom','dfd-native') => 'left bottom',
				esc_html__('Right Top','dfd-native') => 'right top',
				esc_html__('Right Center','dfd-native') => 'right center',
				esc_html__('Right Bottom','dfd-native') => 'right bottom',
				esc_html__('Center Top','dfd-native') => 'center top',
				esc_html__('Center Center','dfd-native') => 'center center',
				esc_html__('Center Bottom','dfd-native') => 'center bottom'
			);
		}
		/**
		...
		public static function dfd_woo_product_thumbs() {
			...
			$lightbox_html = $image_src = $thumbnail_id = $preview_thumb_id = '';
			$preview_thumb_src = $second_image_src = array();
...
//			if(function_exists('wc_get_image_size')) {
				$image_size = wc_get_image_size('thumbnail');//shop_catalog
//			} else {
...
//			}
			...
			$lazyLoad = isset($dfd_native['enable_images_lazy_load']) && $dfd_native['enable_images_lazy_load'] == 'on';
			
			echo '<div id="'. esc_attr($unique_id) .'" class="woo-entry-thumb '.($lazyLoad ? 'dfd-img-lazy-load' : '').'">';
				if(!empty($attachment_ids)) {
					...
					foreach ( $attachment_ids as $attachment_id ) {
						...
						if(!$image_url) {
							$image_url = array(get_template_directory_uri() . '/assets/images/no_image_resized_120-120.jpg', 675, 450);
						}
						
						if($image_size['crop'] !== 0) {
							$image_src = dfd_aq_resize($image_url[0], $imgWidth, $imgHeight, $image_size['crop'], false, true);
						}
						...
						if($attachment_id != $thumbnail_id) {
							...
							$lightbox_html .= '<a href="'.esc_url($image_src[0]).'" '. $data_thumb .' data-rel="prettyPhoto[product-gallery-'. esc_attr($post->ID) .']"></a>';
						}
				...
				if(!$preview_thumb_src || empty($preview_thumb_src)) {
					$preview_thumb_src[0] = get_template_directory_uri() . '/assets/images/no_image_resized_675-450.jpg';
					$preview_thumb_src[1] = 675;
					$preview_thumb_src[2] = 450;
				}

				if(!$second_image_src || empty($second_image_src)) {
					$second_image_src[0] = get_template_directory_uri() . '/assets/images/no_image_resized_675-450.jpg';
					$second_image_src[1] = 675;
					$second_image_src[2] = 450;
				}
				
				if($lazyLoad) {
					$loading_img_src = "data:image/svg+xml;charset=utf-8,%3Csvg xmlns%3D'http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg' viewBox%3D'0 0 $preview_thumb_src[1] $preview_thumb_src[2]'%2F%3E";
					$preview_thumb_html = '<img data-src="'.esc_url($preview_thumb_src[0]).'" src="'. $loading_img_src .'" width="'.esc_attr(floor($preview_thumb_src[1])).'" height="'.esc_attr(floor($preview_thumb_src[2])).'" />';
					$second_image_html = '<img data-src="'.esc_url($second_image_src[0]).'" src="'. $loading_img_src .'" width="'.esc_attr(floor($second_image_src[1])).'" height="'.esc_attr(floor($second_image_src[2])).'" />';
				} else {
					$preview_thumb_html = '<img src="'.esc_url($preview_thumb_src[0]).'" width="'.esc_attr(floor($preview_thumb_src[1])).'" height="'.esc_attr(floor($preview_thumb_src[2])).'" />';
					$second_image_html = '<img src="'.esc_url($second_image_src[0]).'" width="'.esc_attr(floor($second_image_src[1])).'" height="'.esc_attr(floor($second_image_src[2])).'" />';
				}

				echo '<div class="preview-thumb">';
					echo $preview_thumb_html;
				echo '</div>';
					
				echo '<div class="woo-entry-thumb-carousel">';
					$image = $second_image_html;
...
//			return true;
		...
		public static function getSystemStatus() {
			$status = array(
				'memory_limit' => array(
					'name' => esc_html__('Memory limit', 'dfd-native'),
					'value' => esc_html__('No info', 'dfd-native'),
					'passed' => false
				),
				'upload_max_filesize' => array(
					'name' => esc_html__('Upload max file size', 'dfd-native'),
					'value' => esc_html__('No info', 'dfd-native'),
					'passed' => false
				),
				'post_max_size' => array(
					'name' => esc_html__('Post max size', 'dfd-native'),
					'value' => esc_html__('No info', 'dfd-native'),
					'passed' => false
				),
				'max_execution_time' => array(
					'name' => esc_html__('Time limit', 'dfd-native'),
					'value' => esc_html__('No info', 'dfd-native'),
					'passed' => false
				),
				'max_input_vars' => array(
					'name' => esc_html__('Max input vars', 'dfd-native'),
					'value' => esc_html__('No info', 'dfd-native'),
					'passed' => false
				),
			);
			...
			$phpVersionPass = version_compare($phpVersion, '7.4.0', '>');
			if(!$phpVersionPass) {
				$i++;
			}
			
			$status['phpVersion'] = array(
				'name' => esc_html__('PHP version', 'dfd-native'),
				'value' => $phpVersion,
...
//if(!class_exists('Dfd_Theme_Helpers_Elementor')) {
//	/**
//	 * Theme core helpers class
//	 *
//	 *
...
//	 * @author 		DFD
//	 */
//	class Dfd_Theme_Helpers_Elementor {
//		/**
//		 * Returns array of pre-defined Elementor icon packages
//		 *
//		 * @return array
//		 */
//		public static function elementor_icon_fonts() {
//			return array(
//				'fontawesome'	=> esc_html__('Awesome','dfd-native'),
//				'openiconic'	=> esc_html__('Open Iconic','dfd-native'),
//				'typicons'		=> esc_html__('Typicons','dfd-native'),
...
//		
//		/**
//		 * Returns list of Elementor icon packages
//		 *
//		 * @param bool $none
//		 *
//		 * @return array
//		 */
//		public static function build_elementor_icons_fonts_list($none = true) {
//			global $dfd_native;
//			
//			$default_vc_icons = self::elementor_icon_fonts();
//			$font_values = array();
//			
//			if($none) {
//				$name = '';
//				$font_values[$name] = esc_html__('None', 'dfd-native');
//			}
//			
//			$prebuilt = 'dfd_icons';
//			$font_values[$prebuilt] = esc_html__('Standard', 'dfd-native');
//			
//			if(!empty($default_vc_icons) && is_array($default_vc_icons)) {
//				foreach($default_vc_icons as $name => $title) {
//					if(!isset($dfd_native[$name]) || $dfd_native[$name] == 'on') {
//						$font_values[$name] = $title;
//					}
//				}
//			}
//			
