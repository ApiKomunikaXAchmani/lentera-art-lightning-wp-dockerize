...
if(!class_exists('Radium_Theme_Importer')) {
	...
	class Radium_Theme_Importer {
		...
		public function __construct() {
			...
			if($this->external_import === true) {
				$this->content_demo = download_url('http://dfd.name/check-native/files/ntv/'. $this->demo_data_directory .'/'.$this->content_demo_file_name);
				$this->theme_options_file = download_url('http://dfd.name/check-native/files/ntv/'. $this->demo_data_directory .'/'.$this->theme_options_file_name);
				if(!empty($this->widgets_file_name)) {
					$this->widgets = download_url('http://dfd.name/check-native/files/ntv/'. $this->demo_data_directory .'/'.$this->widgets_file_name);
				}
			} else {
		...
		public function is_elementor_demo($file) {
		...
		public function set_demo_data($file) {
			if($this->is_elementor_demo($file)) {
				$this->set_elementor_demo($file);
			} else {
				$this->set_default_demo($file);
			}
		}
		
		public function set_default_demo($file) {
			...
			if(!class_exists('WXR_Importer')) {
				...
				$class_wp_import = dirname(__FILE__) . '/WordPress-Importer-master/plugin.php';
			...
			if($importer_error) {
				die(__('Import error', 'dfd-native'));
			} else {
				if(!is_file($file)) {
					echo __("The XML file containing the dummy content is not available or could not be read. You might want to try to set the file permission to chmod 755. If this doesn't work please use the Wordpress importer and import the XML file (should be located in your download .zip: Sample Content folder) manually", 'dfd-native');
				} else {
		...
		public function set_elementor_demo($file) {
//			if(class_exists('Elementor\Core\App\Modules\ImportExport\Wp_Cli')) {
//				$class_elementor = plugins_url().'/elementor/elementor.php';
//				if(file_exists($class_elementor)) {
//					require_once $class_elementor;
//				} else {
//					die(__('Elementor plugin not found', 'dfd-native'));
//				}
//			}
//			
//			use  as DfdElementorImport;
//			
		...
		public function set_demo_theme_options($file) {
			...
			// File exists?
			if(!file_exists($file) && !$this->external_import) {
				wp_die(
					__('Theme options Import file could not be found. Please try again.', 'dfd-native'), '', array('back_link' => true)
				);
			}
			...
			if(!empty($data) || is_array($data)) {
				...
				die(__('Something went wrong', 'dfd-native'));
			}
		...
		function process_widget_import_file($file) {
			...
			if(!file_exists($file) && !$this->external_import) {
				wp_die(
					__('Widget Import file could not be found. Please try again.', 'dfd-native'), '', array('back_link' => true)
				);
			}
		...
		public function import_widgets($data) {
			...
			foreach($data as $sidebar_id => $widgets) {
				...
				if(isset($wp_registered_sidebars[$sidebar_id])) {
					...
					$sidebar_message = esc_html__('Sidebar does not exist in theme (using Inactive)', 'dfd-native');
				}
				...
				foreach($widgets as $widget_instance_id => $widget) {
					...
					if(!$fail && !isset($available_widgets[$id_base])) {
						...
						$widget_message = esc_html__('Site does not support widget', 'dfd-native'); // explain why widget not imported
					}
					...
					if(!$fail && isset($widget_instances[$id_base])) {
						...
						foreach($single_widget_instances as $check_id => $check_widget) {
							...
							if(in_array("$id_base-$check_id", $sidebar_widgets) && (array) $widget == $check_widget) {
								...
								$widget_message = esc_html__('Widget already exists', 'dfd-native'); // explain why widget not imported
					...
					if(!$fail) {
						...
						if($sidebar_available) {
							...
							$widget_message = esc_html__('Imported', 'dfd-native');
						} else {
							$widget_message_type = 'warning';
							$widget_message = esc_html__('Imported to Inactive', 'dfd-native');
						}
					}

					// Result for widget instance
					$results[$sidebar_id]['widgets'][$widget_instance_id]['name'] = isset($available_widgets[$id_base]['name']) ? $available_widgets[$id_base]['name'] : $id_base; // widget name or ID if name not available (not supported by site)
					$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = isset($widget->title) ? $widget->title : esc_html__('No Title', 'dfd-native'); // show "No Title" if widget instance is untitled
