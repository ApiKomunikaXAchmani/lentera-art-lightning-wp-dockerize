...
(function($, dfd){
	...
	dfd.bindEvent = function() {
		...
		$('body').on('change', dfd.classMain, function() {
			...
			dfd.update();
			////
			dfd.typeOption = dfd.selectedElm.find(":selected").attr("value");
		 ...
		 */
		$('body').on('change', '.form_elem input, .form_elem textarea', function() {
			dfd.initUpdate($(this));
		});
		/* Event on change Style Preset*/
		$('body').on('change', "select[name='preset']", function() {
			val = $(this).val();
