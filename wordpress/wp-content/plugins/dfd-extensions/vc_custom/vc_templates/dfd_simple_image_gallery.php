		...
		$output .= '<div class="dfd-simple-image-gallery '.esc_attr($el_class).'" '.$data_atts.'>';
			foreach($image_ids as $id) {
				...
				if($images_style == 'masonry') {
					if(isset($img_src[1]) && !empty($img_src[1]) && isset($img_src[2]) && !empty($img_src[2])) {
						$width = $img_src[1];
						$height = $img_src[2];
					}
				...
				if(isset($lightbox) && $lightbox == 'true') {
					...
					if(!$thumb_src) {
						$thumb_src = array(get_template_directory_uri() . '/assets/images/no_image_resized_120-120.jpg');
					}
